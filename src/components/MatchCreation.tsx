      ...
      <div className="max-w-md mx-auto">
        ...
        {step === 1 && (
          <Card className="shadow-cricket animate-slide-up bg-orange-800 bg-opacity-70 backdrop-blur-sm border border-orange-600/50">
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={() => setStep(2)} disabled={!matchData.teamA || !matchData.teamB} className="w-full bg-primary hover:opacity-90 transition-all duration-300">
                Next: Match Details
              </Button>
            </CardContent>
          </Card>
        ...
        {step === 2 && (
          <Card className="shadow-cricket animate-slide-up bg-orange-800 bg-opacity-70 backdrop-blur-sm border border-orange-600/50">
            ...
            <CardContent className="space-y-4">
              <div>
                <label className="text-sm font-medium mb-2 block text-gray-300">Number of Players per Team (Min 4)</label>
                <Input
                  type="number"
                  min={4}
                  max={11}
              ...
              <div>
                <label className="text-sm font-medium mb-3 block text-gray-300">Overs</label>
                ...
                <Input
                  ...
                  className="border-2 focus:border-primary bg-gray-700 text-white mt-2"
                />
              </div>

              {matchData.teamSize >= 4 && matchData.overs > 0 && (
                <div className="bg-blue-900/30 p-3 rounded border border-blue-500/50 text-sm">
                  <p className="font-semibold text-blue-300">Match Feasibility:</p>
                  <ul className="list-disc list-inside mt-1 space-y-1 text-gray-300">
                    <li>Bowlers per team: <span className="text-white font-bold">{matchData.teamSize - 1}</span> (1 WK)</li>
                    <li>Max overs per bowler: <span className="text-white font-bold">{Math.ceil(matchData.overs / (matchData.teamSize - 1))}</span></li>
                  </ul>
                </div>
              )}
          ...
          <Card className="shadow-cricket animate-slide-up bg-orange-800 bg-opacity-70 backdrop-blur-sm border border-orange-600/50">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium block text-gray-300">{matchData.teamA || "Team A"} Players ({matchData.teamAPlayers.length}/{matchData.teamSize})</label>
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {matchData.teamAPlayers.map((player: Player, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between bg-[#03045E] text-white p-2 rounded text-sm shadow-sm transition-all hover:brightness-110">
                        <span className="truncate mr-2 font-medium">{player.name}</span>
                        <div className="flex gap-1">
                          {(!isCaptainSelected || player.isCaptain) && (
                            <button onClick={() => toggleCaptainA(player.name)} className={`px-2 py-0.5 rounded text-xs font-bold transition-colors ${player.isCaptain ? 'bg-orange-600 text-white border border-orange-400 shadow-md transform scale-105' : 'bg-orange-300 text-orange-900 hover:bg-orange-400'}`}>C</button>
                          )}
                          {(!isWKSelected || player.isWicketKeeper) && (
                            <button onClick={() => toggleWK_A(player.name)} className={`px-2 py-0.5 rounded text-xs font-bold transition-colors ${player.isWicketKeeper ? 'bg-orange-600 text-white border border-orange-400 shadow-md transform scale-105' : 'bg-orange-300 text-orange-900 hover:bg-orange-400'}`}>WK</button>
                          )}
                        </div>
                      </div>
                    );
              ...
              {/* Team B Input */}
              <div className="space-y-2">
                <label className="text-sm font-medium block text-gray-300">{matchData.teamB || "Team B"} Players ({matchData.teamBPlayers.length}/{matchData.teamSize})</label>
                <div className="flex gap-2">
                  <Input
                    placeholder="Player Name"
                  ...
                  {matchData.teamBPlayers.map((player: Player, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between bg-[#03045E] text-white p-2 rounded text-sm shadow-sm transition-all hover:brightness-110">
                        ...
                        <div className="flex gap-1">
                          {(!isCaptainSelected || player.isCaptain) && (
                            <button onClick={() => toggleCaptainB(player.name)} className={`px-2 py-0.5 rounded text-xs font-bold transition-colors ${player.isCaptain ? 'bg-orange-600 text-white border border-orange-400 shadow-md transform scale-105' : 'bg-orange-300 text-orange-900 hover:bg-orange-400'}`}>C</button>
                          )}
                          {(!isWKSelected || player.isWicketKeeper) && (
                            <button onClick={() => toggleWK_B(player.name)} className={`px-2 py-0.5 rounded text-xs font-bold transition-colors ${player.isWicketKeeper ? 'bg-orange-600 text-white border border-orange-400 shadow-md transform scale-105' : 'bg-orange-300 text-orange-900 hover:bg-orange-400'}`}>WK</button>
                          )}
                        </div>
                      </div>
                    );
            ...
            <CardContent className="space-y-4">
              ...
              {tossDetails.call && !tossDetails.result && (
                <>
                  ...
                  <Button onClick={handleToss} className="w-full bg-primary hover:opacity-90 transition-all duration-300" disabled={!tossDetails.flippingTeam || !tossDetails.callingTeam || !tossDetails.call || coinState !== 'idle'}>
              ...
              {tossDetails.result && (
                <div className="text-center space-y-2 mt-4">
                  ...
                  {matchData.tossDecision && (
                    <>
                      <div className="p-4 bg-accent rounded-lg mt-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm text-muted-foreground">Public Match Code:</p>
                            <div className="flex items-center gap-2">
                              <p className="font-mono text-xl font-bold text-cricket-field tracking-wider">{publicCodeRef.current}</p>
                              <Button
                                variant="ghost"
                                size="sm"
                        ...
                        <div className="pt-2 border-t border-white/10">
                          ...
                          <div className="flex items-center gap-2">
                            <p className="font-mono text-lg text-cricket-field">{umpireKeyRef.current}</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-8 w-8 p-0 text-gray-400 hover:text-white"
            ...
            <CardContent className="space-y-4">
              <div className="text-center space-y-3">
                <div className="p-4 bg-accent rounded-lg">
                  ...
                  <p className="text-sm mt-2">{matchData.tossWinner} won the toss and decided to {matchData.tossDecision} first.</p>
