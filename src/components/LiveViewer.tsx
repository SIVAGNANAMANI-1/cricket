  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex border-b border-gray-700 mb-4">
        <button
          className={`flex-1 pb-2 text-center font-semibold text-sm transition-colors relative ${activeTeam === matchData.teamA ? "text-cricket-field" : "text-gray-500 hover:text-gray-700"
            }`}
          onClick={() => setActiveTeam(matchData.teamA)}
        >
          {matchData.teamA}
          {activeTeam === matchData.teamA && (
            <div className="absolute bottom-0 left-0 w-full h-0.5 bg-cricket-field rounded-t-full" />
          )}
        </button>
        <button
          className={`flex-1 pb-2 text-center font-semibold text-sm transition-colors relative ${activeTeam === matchData.teamB ? "text-cricket-field" : "text-gray-500 hover:text-gray-700"
            }`}
      ...
      <Card className="shadow-cricket bg-[#1e293b] border-gray-700 text-white">
        <CardHeader>
...
export const LiveViewer = ({ matchData, onBack, score, striker, nonStriker, currentBowler, battingTeam, bowlingTeam, bowlerOvers, allPlayers, totalDotBalls, totalFours, totalSixes, ballHistory, currentOverRuns, currentOverBalls, currentInnings, innings1Score, matchResult }: LiveViewerProps) => {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white p-4 flex items-center justify-center">
      <div className="w-full max-w-md">
        ...
        {/* Match Header */}
        <Card className="shadow-xl mb-4 animate-fade-in bg-[#1e293b] border-gray-700 text-white">
          <CardContent className="p-4">
            ...
            <div className="text-center mb-3">
              ...
              <Badge variant="secondary" className="bg-gray-700 text-gray-200 hover:bg-gray-600">{matchData.overs} Overs Match</Badge>
            ...
            <div className="text-center">
              ...
              {currentInnings === 2 && innings1Score !== null && !matchResult && (() => {
                ...
                return (
                  <div className={`mt-2 p-3 border rounded-md ${isLost ? 'bg-red-900/20 border-red-700/50' : 'bg-orange-600/20 border-orange-600/50'}`}>
                    {isStartOfInnings2 && !isLost && (
                      <p className="text-sm font-bold text-orange-400 mb-1 uppercase tracking-wide">
              ...
              <div className="mt-4 mb-2">
                <div className="text-xs text-gray-400 mb-2 uppercase tracking-wide text-left">Recent Balls</div>
                <div className="flex flex-nowrap gap-2 justify-start overflow-x-auto pb-2 scrollbar-none">
                  {ballHistory.slice(-12).map((ball: any, index: number, arr: any[]) => {
                    ...
                    return (
                      <div key={index} className="flex items-center">
                        {showPipe && <span className="text-gray-500 mx-1 text-lg">|</span>}
                        <div
                          className={`w-8 h-8 flex items-center justify-center rounded-full text-xs font-bold border ${isWicket ? 'bg-red-500/20 border-red-500 text-red-500' :
                            isBoundary ? 'bg-green-500/20 border-green-500 text-green-500' :
                  ...
                  {ballHistory.length === 0 && <span className="text-xs text-gray-500">No balls yet</span>}
                </div>
              </div>
              ...
              {(striker || nonStriker) && (() => {
                ...
                return (
                  <div className="mt-3 pt-3 border-t border-gray-700">
                    <div className="text-xs text-gray-400 mb-2 uppercase tracking-wide">Batting</div>
                    <div className="space-y-1">
                      {striker && (
                        <div className="flex justify-between items-center text-sm">
                          <span className="font-medium text-white">{striker.name}*</span>
                      ...
                      {nonStriker && (
                        <div className="flex justify-between items-center text-sm">
                          <span className="font-medium text-white">{nonStriker.name}</span>
                          <span className="text-white font-bold">
        ...
        {/* Tab Navigation */}
        <div className="flex mb-4 bg-[#1e293b] rounded-lg p-1 shadow-lg border border-gray-700">
          {[
        ...
        {activeTab === 'commentary' && (
          <div className="space-y-4">
            <Card className="shadow-xl bg-[#1e293b] border-gray-700 text-white">
              <CardHeader>
                <CardTitle className="text-sm text-white">Live Commentary</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                    ...
                    .map((ball: any, index: number) => {
                      ...
                      return (
                        <div key={index} className="p-2 rounded border-l-4" style={{
                          ...
                          <p className="font-medium text-xs text-gray-400 mb-1">{overFormatted} overs</p>
